...
#include "stream_fit_validator.hxx"
#include "validator.h"
...
#define CONFIG_FILE_NAME "../validator_config.txt"
...
//helper function that grabs the tolerance from the settings file
double get_tolerance(std::string configfilename) {
    ...
    if(!configfile.is_open())
        {
            log_messages.printf(MSG_CRITICAL, "Could not open config file\n");
            exit(1);
        }

    string temp;
    double tolerancebound;
    configfile >> temp >> tolerancebound;

    if(!scanf("%lf", &tolerancebound) == 1)
        {
            log_messages.printf(MSG_CRITICAL, "Provided tolerance bound must be a float\n");
            exit(1);
        }
...
int check_set(vector<RESULT>& results, WORKUNIT& wu, long& canonicalid, double&, bool& retry) {
    ...
    double tolerancebound = get_tolerance(CONFIG_FILE_NAME);
    ...
    for (uint32_t i = 0; i < results.size(); i++) {
        try {
            ...
            log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([RESULT#%ld %s]) failed with error: %s\n", results[i].id, results[i].name, error_message.c_str());
            had_error[i] = true;
    ...
    try {
        ...
        log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([WORKUNIT#%ld %s]) failed with error: %s\n", wu.id, wu.name, error_message.c_str());
        exit(1);
    }
...
//    cout << "search_name: " << search_name << endl;
    ...
    // APPARENTLY it's possible for the WU generator to bundle the wrong number of tasks
    // If the parameters list is too long, it will cause a DB error
    // this checks for that and exits before DB things to avoid crashing the validator
    if (strlen(vector_to_string(result_parameters).c_str()) > 2048) {
        log_messages.printf(MSG_CRITICAL, "encountered WU with too many parameters (too many bundled tasks?): skipping this WU and marking all results invalid\n");
        
	//mark all results invalid
        for (uint32_t i = 0; i < results.size(); i++) {
	    results[i].outcome = RESULT_OUTCOME_VALIDATE_ERROR;
    ...
    try {
        if (searches.find(search_name) == searches.end()) {
            ...
            if (search_name.substr(0,3).compare("ps_") == 0) {
                ...
                log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([WORKUNIT#%ld %s]) had an unkown search name (either removed from database or needs to start with de_ or ps_): '%s'\n", wu.id, wu.name, search_name.c_str());
                ea = NULL;
            }
    ...
    if (results.size() == 1) {  //This was the first result back for the workunit
        if (had_error[0]) {
            log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([RESULT#%ld %s]) had an error was marked as invalid, and there are no other results for this workunit.\n", results[0].id, results[0].name);
        ...
        if (ea == NULL || !ea->would_insert(position, result_fitness[0])) {  //This is a result we aren't going to use, so check to see if we need to use adaptive validation, and just mork it valid otherwise
            ...
            if (!retval) {
                ...
                if (r_num < success_rate) {
                    log_messages.printf(MSG_DEBUG, "ea_validation_policy check_set([RESULT#%ld %s]), single result, r_num %lf < success_rate %lf, marking valid.\n", results[0].id, results[0].name, r_num, success_rate);
                    results[0].validate_state = VALIDATE_STATE_VALID;
                    canonicalid = results[0].id;
                } else {
                    log_messages.printf(MSG_DEBUG, "ea_validation_policy check_set([RESULT#%ld %s]), single result, r_num %lf >= success_rate %lf, marking inconclusive.\n", results[0].id, results[0].name, r_num, success_rate);
                }
            } else {
                    log_messages.printf(MSG_DEBUG, "ea_validation_policy check_set([RESULT#%ld %s]), single result, going to be inserted into EA, marking inconclusive for validation.\n", results[0].id, results[0].name);
            }
            ...
            if (mysql_errno(boinc_db.mysql) != 0) {
                log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([WORKUNIT#%ld %s]) could not update workunit min_quorum field. Error: %d -- '%s'\n", wu.id, wu.name, mysql_errno(boinc_db.mysql), mysql_error(boinc_db.mysql));
                exit(1);
            }
...
//            wu.min_quorum++;    //may be able to just do this
        ...
        for (uint32_t i = 0; i < results.size(); i++) {
            ...
            for (uint32_t j = 0; j < results.size(); j++) {
                ...
                if (fabs(result_fitness[i] - result_fitness[j]) < tolerancebound) {
                    matches++;
                }
            }
        ...
        if (canonical_resultid > 0) {
            ...
            for (uint32_t i = 0; i < results.size(); i++) {
                if (fabs(result_fitness[i] - canonical_fitness) < tolerancebound) {
                    results[i].validate_state = VALIDATE_STATE_VALID;
                    log_messages.printf(MSG_DEBUG, "              fitness: %lf -- marked valid,   result_id: %ld.\n", result_fitness[i], results[i].id);
                } else {
                    results[i].outcome = RESULT_OUTCOME_VALIDATE_ERROR;
                    results[i].validate_state = VALIDATE_STATE_INVALID;
                    log_messages.printf(MSG_DEBUG, "              fitness: %lf -- marked INVALID, result_id: %ld.\n", result_fitness[i], results[i].id);
                }
            }
...
void check_pair(RESULT& new_result, RESULT& canonical_result, bool& retry) {
    ...
    try {
        ...
        log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_pair([RESULT#%ld %s]) failed with error: %s\n", new_result.id, new_result.name, error_message.c_str());
    ...
    try {
        ...
        log_messages.printf(MSG_CRITICAL, "ea_validation_policy check_set([RESULT#%ld %s]) failed with TERMINAL error: %s -- COULD NOT PARSE CANONICAL RESULT XML\n", canonical_result.id, canonical_result.name, error_message.c_str());
        exit(1);
    }

    if (fabs(new_fitness - canonical_fitness) < tolerancebound) {
